...
int JXYUVEncodeH264::initVideoEncoder() {
    ...
    char *out_file = (char *) malloc(path_length + 1);  // malloc  动态分配内存空间
    strcpy(out_file,
           arguments->video_path);   // /storage/emulated/0/DCIM/mabeijianxi/1546481824016/1546481824016.h264
    logi( "视频编码器初始化开始-------%s", out_file)
    av_register_all();
    ...
    if (avio_open(&pFormatCtx->pb, out_file, AVIO_FLAG_READ_WRITE) < 0) {
        logi("", "_Failed to open output file! \n");
        return -1;
    }
...
void *JXYUVEncodeH264::startEncode(void *obj) {
    ...
    while (!h264_encoder->is_end || !h264_encoder->frame_queue.empty()) {
        ...
        logi("send_videoframe_count:%d", h264_encoder->frame_count);
...
//            uint8_t *aa=h264_encoder->pkt.data;
//            char* bbbbb = (char*)aa;
//
//            h264_encoder->pkt.
//            LOGI(JNI_DEBUG, "视频编码器初始化结束时间uint8_t-------%s\n", bbbbb)


            h264_encoder->framecnt++;
            h264_encoder->pkt.stream_index = h264_encoder->video_st->index;
            // &h264_encoder->pkt 等待输出
            ret = av_write_frame(h264_encoder->pFormatCtx, &h264_encoder->pkt);

            //
            h264_encoder->arguments->handler->backByte(h264_encoder->arguments,h264_encoder->pkt.size);
